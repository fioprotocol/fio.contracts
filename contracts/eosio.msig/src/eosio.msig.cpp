                        ...
                        ignore<transaction> trx )
{
   ...
   eosiosystem::top_producers_table  topprods(SYSTEMACCOUNT, SYSTEMACCOUNT.value);
   auto tpiter = topprods.find(proposer.value);
   ...
   if(!(   proposer == fioio::MSIGACCOUNT ||
           proposer == fioio::WRAPACCOUNT ||
           proposer == fioio::SYSTEMACCOUNT ||
           proposer == fioio::ASSERTACCOUNT ||
           proposer == fioio::REQOBTACCOUNT ||
           proposer == fioio::FeeContract ||
           proposer == fioio::AddressContract ||
           proposer == fioio::TPIDContract ||
           proposer == fioio::TokenContract ||
           proposer == fioio::FOUNDATIONACCOUNT ||
           proposer == fioio::TREASURYACCOUNT ||
           proposer == fioio::FIOSYSTEMACCOUNT ||
           proposer == fioio::FIOACCOUNT ||
           isTopProd)
           ) {
       //collect fees.
       eosio::action{
               ...
               std::make_tuple(std::string("msig_propose"), proposer, max_fee, sizep)
       }.send();
   }
   ...
   apptable.emplace( proposer, [&]( auto& a ) {
      ...
      if (PROPOSERAM > 0) {
          ...
          action(
                  ...
                  "eosio"_n,
                  "incram"_n,
                  std::make_tuple(proposer, rambump)
          ).send();
      }
